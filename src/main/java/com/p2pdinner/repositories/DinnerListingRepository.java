...
public interface DinnerListingRepository extends JpaRepository<DinnerListing, Integer> {
    ...
    @Query("SELECT DISTINCT d FROM DinnerListing d WHERE d.menuItem.userProfile.id = :profileId AND d.closeTime > CURRENT_TIMESTAMP AND d.availableQuantity > 0 ")
	Collection<DinnerListing> currentListings(@Param("profileId") Integer profileId);
