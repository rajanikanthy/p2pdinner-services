...
public class CloseTimeAfterFilter implements Predicate {
    ...
    public boolean evaluate(Object o) {
        if ( o instanceof DinnerListing) {
            ...
            LOGGER.debug("Dinner Listing Item : {}", dinnerListing.getMenuItem().getTitle());
            LOGGER.debug("Dinner Listing close Time  {}", formatter.print(listingCloseTime));
            LOGGER.debug("Query Time {}", formatter.print(dateTime));
            if (listingCloseTime.isAfter(dateTime)) {
                LOGGER.debug(" <<FILTER> CloseTimeFilterAfter returning true");
                return true;
            }
        }
        return false;
